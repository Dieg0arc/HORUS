...
def extract_keypoints(pose_result, left_hand_result, right_hand_result, face_result) -> np.ndarray:
    ...
    face = _landmarks_to_array(face_result.face_landmarks[0] if face_result and face_result.face_landmarks else None, 468)
...
def process_video(video_path: Path, keypoints_dir: Path):
    ...
    with PoseLandmarker.create_from_options(pose_options) as pose_landmarker, \
        ...
        while len(keypoints_sequence) < SEQUENCE_LENGTH and cap.isOpened():
            ...
            keypoints = extract_keypoints(pose_result, left_hand_result, right_hand_result, face_result)
...
def main():
    ...
    models = ['pose_landmarker.task', 'hand_landmarker.task', 'face_landmarker.task']
    ...
    if missing_models:
        ...
        print("- Face: https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/2/face_landmarker.task")
